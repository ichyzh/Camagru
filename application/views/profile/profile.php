            ...
            <nav aria-label="..." class="m-auto">
                <ul class="pagination pagination-lg">
                    <?php for($i = 0; $i < $pag['number_of_pages']; $i++): ?>
                        <li class="page-item">
                            <a class="page-link pag-link" href="/<?php echo ROOT ?>/profile/<?php echo $vars[0]['login'] ?>?page=<?php echo $i+1 ?>" tabindex="-1"><?php echo $i+1 ?></a>
                        </li>
                    <?php endfor; ?>
                </ul>
            </nav>
                                            ...
                                            <?php if($logged['this_user']): ?>
                                                <div class="row float-right">
                                                    <?php if ($val['liked']): ?>
                                                        ...
                                                        <?php echo "<button class='btn like liked' id='like' value=" . $val['id'] . "><i class='far fa-heart pr-1 io_size'  ><p class='num_likes_mod'>" . $val['likes_count'] . "</p></i></button>" ?>
                                                        <?php echo "</div>" ?>
                                                    <?php else: ?>
                                                        <?php echo "<div class=\"col-5 text-right\">" ?>
                                                        <?php echo "<button class='btn like' id='like' value=" . $val['id'] . "><i class='far fa-heart pr-1 io_size'  ><p class='num_likes_mod'>" . $val['likes_count'] . "</p></i></button>" ?>
                                                        <?php echo "</div>" ?>
                                                    <?php endif; ?>
                                                    <div class="col-5 text-right">
                                                        <button class="btn like"><span style="font-size: 1rem"><i class="far fa-comment-alt io_size" ></i><?php echo $val['comments_count'] ?></span></button>
                                                    </div>
                                                </div>
                                            <?php else: ?>
                                                <div class="row float-right">
                                                    <?php if ($val['liked']): ?>
                                                        <?php echo "<div class=\"col-5 \">" ?>
                                                        <?php echo "<button class='btn like liked' id='like' value=" . $val['id'] . "><i class='far fa-heart pr-1 io_size' ><p class='num_likes_mod'>" . $val['likes_count'] . "</p></i></button>" ?>
                                                        <?php echo "</div>" ?>
                                                    <?php else: ?>
                                                        <?php echo "<div class=\"col-5 \">" ?>
